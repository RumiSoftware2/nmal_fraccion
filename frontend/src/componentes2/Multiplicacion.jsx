...
export default function Multiplicacion({ result1, result2, base1, base2 }) {
  ...
  const handleMultiplicacion = async () => {
    if (!mismaBase) {
      setError('❌ Las fracciones deben estar en la misma base para multiplicarlas')
      return
    }
    ...
    try {
      ...
      console.error('Error en multiplicación:', err)
      setError(err.message || 'Error al realizar la multiplicación')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <motion.div
      className="multiplicacion-container"
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.4 }}
    >
      <div className="multiplicacion-header">
        <h3>✖️ Multiplicación de Fracciones</h3>
      ...
      {!mismaBase && (
        ...
        >
          ...
          <p>Para multiplicar, deben estar en la misma base.</p>
        </motion.div>
      )}

      {/* Card de operación */}
      <div className="multiplicacion-operation">
        {/* Fracción 1 */}
      ...
      {/* Botones de acción */}
      <div className="multiplicacion-actions">
        <motion.button
      ...
      <AnimatePresence mode="wait">
        ...
        {activeView === 'resultado' && resultado && (
          <motion.div
            key="resultado"
            className="resultado-multiplicacion"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -15 }}
